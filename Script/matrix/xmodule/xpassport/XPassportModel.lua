...
function XPassportModel:SetPayCallBack(value)
...
--日服新增
function XPassportModel:PassportBuyPassportResponse(data)
    if data.Code ~= XCode.Success then
        XUiManager.TipCode(data.Code)
        return
    end

    local name = self:GetPassportTypeInfoConfig(data.PassportInfo.Id).Name or ""
    local msg = CS.XTextManager.GetText("SuccessfulItemPurchase", name)
    XUiManager.TipMsg(msg)

    if not XTool.IsTableEmpty(data.RewardList) then
        XUiManager.OpenUiObtain(data.RewardList)
    end
    self:UpdatePassportInfosDic({data.PassportInfo})

    if self.PayCallBack and XLuaUiManager.IsUiShow("UiPassportCard") then
        self.PayCallBack()
        self.PayCallBack = nil
    end
