...
function XLoginManager.DoDisconnect(text)
    ...
    if XDataCenter.FunctionEventManager.CheckFuncDisable() then
        ...
        XUiManager.SystemDialogTip(CS.XTextManager.GetText("TipTitle"), CS.XTextManager.GetText("HeartbeatTimeout"), XUiManager.DialogType. OnlySure, nil, XLoginManager.BackToLogin)
    end
...
function XLoginManager.DoLogin(cb)
    ...
    XLog.Debug("channel:" .. tostring(XUserManager.Channel) .. "," .. tostring(CS.XRemoteConfig.Channel) .. ", userId:" .. tostring(XUserManager.UserId))

    if XUserManager.UserId == nil then
        return
    end
    --第一次登录，初始化
...
XRpc.NotifyLogin = function(data)
    ...
    -- XHgSdkManager.reportAccountInfo(data.PlayerData.ServerId,data.PlayerData.Id,CS.XRemoteConfig.GooglePointOrderUrl,XUserManager.UserId)
