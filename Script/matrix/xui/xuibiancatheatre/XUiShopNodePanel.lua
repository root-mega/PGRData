...
function XUiShopItem:SetData(data)
    ...
    local disCount = (price ~= 0 and discountPrice - price ~= 0) and discountPrice / price or 0
    -- 海外改为 xx% Off
    local display = 0
    if disCount ~= 1 then
        local invertDiscount = (1 - disCount) * 100
        display = math.ceil(invertDiscount)
    end

    local disCountDesc = disCount ~= 1 and string.format("%d", display) or disCount
