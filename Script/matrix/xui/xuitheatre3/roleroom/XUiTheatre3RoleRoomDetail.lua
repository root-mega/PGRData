...
function XUiTheatre3RoleRoomDetail:OnEnable()
    ...
    if self.IsEnableFin then
        self.PanelFilter:DoSelectTag("BtnAll", true, self._CurSelect)
        self:UpdateRoleModel(self._CurSelect)
    end
...
function XUiTheatre3RoleRoomDetail:UpdateRoleInfo(id)
    ...
    self.BtnCharacterDetails.gameObject:SetActiveEx(not XRobotManager.CheckIsRobotId(id))
end
