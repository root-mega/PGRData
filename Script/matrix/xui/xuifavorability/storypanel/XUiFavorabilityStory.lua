...
function XUiFavorabilityStory:RefreshList()
    local config=XFavorabilityConfigs.GetStoryLayout(self.CurrentCharacterId)
...
function XUiFavorabilityStory:RefreshBaseData()
    --self.TxtName.text=XUiHelper.GetText('FavorabilityStoryCharName',XCharacterConfigs.GetCharacterName(self.CurrentCharacterId),XCharacterConfigs.GetCharacterTradeName(self.CurrentCharacterId))
    self.TxtName.text=XMVCA.XCharacter:GetCharacterLogName(self.CurrentCharacterId)
    --队伍图标
    local teamIcon=XFavorabilityConfigs.GetCharacterTeamIconById(self.CurrentCharacterId)
    if teamIcon then
...
function XUiFavorabilityStory:RefreshStoryData()
    ...
    --显示和设置格子数据
    local plotDatas = XFavorabilityConfigs.GetCharacterStoryById(self.CurrentCharacterId)
    ...
    --显示解锁进度
    local unlockNum,storyNum=XDataCenter.FavorabilityManager.StoryUnlockNum(self.CurrentCharacterId)
