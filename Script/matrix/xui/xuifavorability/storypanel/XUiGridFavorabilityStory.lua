local XUiGridFavorabilityStory=XClass(nil,"XUiGridFavorabilityStory")
...
function XUiGridFavorabilityStory:Ctor(ui,parent)
    self.Parent=parent
...
function XUiGridFavorabilityStory:Refresh(data)
    ...
    self.IsUnlock = XDataCenter.FavorabilityManager.IsStoryUnlock(characterId, self.PlotData.Id)
    self.CanUnlock = XDataCenter.FavorabilityManager.CanStoryUnlock(characterId, self.PlotData.Id)
...
function XUiGridFavorabilityStory:OnClickEvent()
    if self.CanUnlock then
        XDataCenter.FavorabilityManager.OnUnlockCharacterStory(self.Parent.CurrentCharacterId, self.PlotData.Id)
