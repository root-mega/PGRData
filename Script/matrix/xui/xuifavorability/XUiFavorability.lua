...
function XUiFavorability:OnBtnMainUIClick()
    self:SetCurrFavorabilityCharacter(nil)
    XLuaUiManager.RunMain()
end

function XUiFavorability:OnBtnMaskClick()
...
function XUiFavorability:CloseOtherViewWhenExchagneRoleOpen()
    self.FavorabilityDocument:SetViewActive(false)
    self.FavorabilityPlot:SetViewActive(false)
    self.FavorabilityGift:SetViewActive(false)
...
-- [切换角色]
function XUiFavorability:OnBtnSwitchClick()
...
function XUiFavorability:PlaySaftyAnimation(animName, endCb, startCb)
