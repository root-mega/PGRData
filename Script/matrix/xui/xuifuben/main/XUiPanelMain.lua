...
function XUiPanelMain:_RefreshWeeklyManagers()
    local managers = {}
    local source_managers = self.FubenManagerEx.GetShowOnMainWeeklyManagers()

    -- 海外会有周常活动关闭而需要屏蔽按钮的情况
    for _, m in pairs(source_managers) do
        if m:OverseaActive() then
