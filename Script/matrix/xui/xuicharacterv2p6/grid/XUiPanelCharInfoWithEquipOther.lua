...
function XUiPanelCharInfoWithEquipOther:UpdateRoleView()
    ...
    -- 职业
    local career = self.CharacterAgency:GetCharacterCareer(character.Id)
    local careerIcon = XCharacterConfigs.GetNpcTypeIcon(career)
    self.BtnType:SetRawImage(careerIcon)

    local showUniframe = self.CharacterAgency:GetIsIsomer(character.Id)
    ...
    -- 战斗参数
    self.TxtFight.text = self.CharacterAgency:GetCharacterAbilityOther(character, self.EquipList, self.AssignChapterRecords, partner)
...
function XUiPanelCharInfoWithEquipOther:UpdatePanelSuitCount()
    ...
    -- 套装列表
    local suitInfoList = self.EquipAgency:GetWearingSuitInfoListByEquipListAndWeapon(self.Awareness, self.Weapon)
    local itemIndex = 1
